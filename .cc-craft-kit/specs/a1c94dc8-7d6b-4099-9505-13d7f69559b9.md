...
- [ ] `get-status.ts` の phases 配列に `'review'` を追加
- [ ] `status.ts` の phases 配列に `'review'` を追加

### Phase 2: 型定義の統一

- [ ] `phase-status-mapper.ts` の Phase 型に `'review'` を追加

### Phase 3: 動作確認

- [ ] 型チェック・リント実行
- [ ] `/cft:status` で review フェーズが正しく表示されることを確認
