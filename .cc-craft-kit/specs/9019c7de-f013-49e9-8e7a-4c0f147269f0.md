...
pr_url: "https://github.com/B16B1RD/cc-craft-kit/pull/954"
created_at: "2025-12-07T04:00:00.000Z"
updated_at: "2025-12-07T07:55:00.000Z"
---
...
- [x] `project_name_cache` を `project_v2` にリネーム
- [x] `project_v2.id` フィールドを追加（Global Node ID）
- [x] Project ID 解決時に `id` を保存するよう更新

### 8.2 github.md の修正

- [x] Issue 作成後に Projects 追加処理を追加
- [x] `gh issue view --json id` で Issue Node ID を取得
- [x] `addProjectV2ItemById` mutation を実行
- [x] エラー時は警告のみで続行

### 8.3 spec.md の修正

- [x] design フェーズの Sub Issue 作成後に Projects 追加処理を追加
- [x] github.md と同じパターンで実装

### 8.4 Project ID 解決処理の追加

- [x] config.json に `project_v2.id` がない場合、GraphQL で取得
- [x] 取得した ID を config.json にキャッシュ
