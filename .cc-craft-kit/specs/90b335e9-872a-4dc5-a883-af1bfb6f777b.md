...
- [ ] 仕様書 4bd4f16b から `create-mcp-error-spec.ts` への参照を削除 (#584)

### Phase 2: ファイル削除

- [ ] `scripts/create-mcp-error-spec.ts` を `git rm` で削除 (#585)

### Phase 3: 環境変数設定の更新

- [ ] `.env.example` から MCP_PORT 関連行（26-27行目）を削除 (#586)
- [ ] `.env` から MCP_PORT 関連行（26-27行目）を削除 (#587)

### Phase 4: 検証

- [ ] 型チェック・テスト・Lintを実行して問題がないことを確認 (#588)
