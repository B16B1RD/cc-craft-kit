...
- [ ] SpecPhase 型に 'review' を追加（schema.ts） (#621)
- [ ] StatusMapping インターフェースに review エントリを追加（github-status-config.ts） (#622)
- [ ] DEFAULT_STATUS_MAPPING を更新（review: In Review, completed: Done） (#623)
- [ ] validateStatusConfig の requiredPhases に review を追加 (#624)
- [ ] PHASE_ALIASES に review 省略形（rev）を追加（phase-mapping.ts） (#625)

### Phase 2: フェーズ遷移ロジック更新

- [ ] implementation → review の遷移ルールを追加（phase-transition-validator.ts） (#626)
- [ ] review → completed の遷移ルールを追加（PR マージ済み確認） (#627)
- [ ] review フェーズ遷移時の GitHub 連携処理を追加（github-integration.ts） (#628)
- [ ] completed フェーズ遷移時に pr-cleanup 処理を統合（github-integration.ts） (#629)
