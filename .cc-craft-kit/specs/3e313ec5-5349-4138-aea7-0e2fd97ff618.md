...
- [ ] `resolve-id.ts` の `resolveSpecById` 関数で `github_number` を SELECT するように修正
- [ ] `resolve-id.ts` の戻り値で `sync?.github_number` を使用するように修正
- [ ] `resolve-id.ts` の `resolveSpecByIssueNumber` 関数で `github_number` を使用するように修正
- [ ] `pull-request.ts` の `syncRecord.issue_number` を `syncRecord.github_number` に修正
...
- [ ] `npm test` で全テスト実行
- [ ] `/cft:spec-phase` で動作確認（GitHub Issue が認識されることを確認）
