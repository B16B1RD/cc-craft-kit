...
- [ ] `getCurrentBranch()` のテスト（正常系、Git コマンド失敗時）
- [ ] `getProtectedBranches()` のテスト（環境変数あり、なし）
- [ ] `isProtectedBranch()` のテスト（保護対象、非保護対象）
- [ ] `validateBranch()` のテスト（保護対象でエラー、非保護対象で成功）
- [ ] `suggestWorkingBranch()` のテスト（各種フェーズ）
- [ ] キャッシュの動作テスト
- [ ] Git コマンドのモック化
