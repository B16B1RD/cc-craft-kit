...
- [ ] `.cc-craft-kit/backups/` ディレクトリを削除する (#796)
- [ ] 削除後に `npm run sync:dogfood` が正常に動作することを確認する (#797)

### Phase 2: スキル重複整理

- [ ] `.cc-craft-kit/skills/` の内容を確認し、用途不明なスキルを特定する (#798)
- [ ] `.claude/skills/` との重複を解消する（不要なスキル定義を削除） (#799)

### Phase 3: 同期スクリプト更新

- [ ] `src/scripts/sync-dogfood.ts` に `backups/` ディレクトリの削除パターンを追加する (#800)
- [ ] 最終動作確認を行う（lint, typecheck, test） (#801)
