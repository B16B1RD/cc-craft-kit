---
id: "842ac6b3-0ccc-46c2-9840-11bbfb89a58d"
name: "implementation フェーズ完了時、未コミットのものがある"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-23T11:47:51.075Z"
updated_at: "2025-11-23T21:45:57Z"
---
...
本セクションでは、受け入れ基準に基づき、実装タスクを実施可能な単位に分解します。

### タスク 1: コア機能の実装

**目的**: `getCommitTargets()` 関数を修正し、全変更ファイルをコミット対象とする

**作業内容**:
- [ ] `src/core/workflow/git-integration.ts:165-174` の `getCommitTargets()` 関数を修正
  - 返り値を `['.cc-craft-kit/specs/${specId}.md']` から `['.']` に変更
  - コメントを「仕様書ファイルのみをコミット対象とする」から「全変更ファイルをコミット対象とする」に更新

**受け入れ基準**:
- ✅ `getCommitTargets()` が `['.']` を返却する
- ✅ UUID バリデーションが維持されている
- ✅ コードがコンパイルエラーなく動作する
...
**目的**: 修正したソースコードを `.cc-craft-kit/` ディレクトリに同期する
...
**受け入れ基準**:
- ✅ `.cc-craft-kit/commands/` 配下にファイルが正しく同期されている
- ✅ 同期処理が成功している
...
**目的**: `getCommitTargets()` 関数の単体テストの期待値を更新する

**作業内容**:
- [ ] `tests/core/workflow/git-integration.test.ts:318-387` を修正
  - 期待値を `['.cc-craft-kit/specs/${specId}.md']` から `['.']` に更新
- [ ] 単体テストを実行: `npm run test -- tests/core/workflow/git-integration.test.ts`
- [ ] テストが通過することを確認
...
**目的**: フェーズ遷移時の自動コミット処理の E2E テストが通過することを確認する

**作業内容**:
- [ ] E2E テストを実行: `npm run test -- tests/e2e/phase-transition-commit.test.ts`
- [ ] テストが通過することを確認
- [ ] 特に `implementation → completed` 時のテストケースを重点的に確認

**受け入れ基準**:
- ✅ E2E テストがすべて通過する
- ✅ `expectCommitTargetsToBe(['.'])` のアサーションが成功する

**依存タスク**: タスク 3（単体テストの修正）

**優先度**: 🔴 高
---

### タスク 5: 型チェックと ESLint チェック

**目的**: コード品質を維持し、型エラーや ESLint 警告がないことを確認する

**作業内容**:
- [ ] 型チェックを実行: `npx tsc --noEmit`
- [ ] ESLint チェックを実行: `npm run lint`
- [ ] エラーや警告がないことを確認

**受け入れ基準**:
- ✅ 型エラーがない
- ✅ ESLint 警告がない
- ✅ TypeScript strict mode が維持されている

**依存タスク**: タスク 4（E2E テストの確認）
...
**目的**: code-reviewer サブエージェントで品質レビューを実施する

**作業内容**:
- [ ] `/cft:code-review src/core/workflow/git-integration.ts` を実行
- [ ] レビュー指摘事項を確認
- [ ] 必要に応じて修正

**受け入れ基準**:
- ✅ code-reviewer の指摘事項がない、または軽微な提案のみ
- ✅ セキュリティ、パフォーマンス、保守性が維持されている

**依存タスク**: タスク 5（型チェックと ESLint チェック）
...
**目的**: 実際のフェーズ遷移で全変更ファイルが自動コミットされることを確認する

**作業内容**:
- [ ] テスト用仕様書を作成: `/cft:spec-create "テスト: 全変更ファイルコミット確認"`
- [ ] 実装ファイルを追加: `echo "console.log('test');" > test-file.ts`
- [ ] `implementation` フェーズに移行: `/cft:spec-phase <test-spec-id> implementation`
- [ ] `completed` フェーズに移行: `/cft:spec-phase <test-spec-id> completed`
- [ ] Git 状態を確認: `git status`
- [ ] テスト用ファイルを削除: `rm test-file.ts`
- [ ] テスト用仕様書を削除: `/cft:spec-delete <test-spec-id> --yes`

**受け入れ基準**:
- ✅ `git status` が `nothing to commit, working tree clean` を表示する
- ✅ 全変更ファイル（仕様書 + 実装ファイル）が自動コミットされる
- ✅ コミットメッセージが既存の規則に従っている

**依存タスク**: タスク 6（コード品質レビュー）
...
1. **タスク 1**: コア機能の実装 (src/core/workflow/git-integration.ts 修正)
2. **タスク 2**: 同期処理の実行 (npm run sync:dogfood)
3. **タスク 3**: 単体テストの修正 (tests/core/workflow/git-integration.test.ts)
4. **タスク 4**: E2E テストの確認 (tests/e2e/phase-transition-commit.test.ts)
5. **タスク 5**: 型チェックと ESLint チェック (npx tsc, npm run lint)
6. **タスク 6**: コード品質レビュー (/cft:code-review)
7. **タスク 7**: 動作確認 (実際のフェーズ遷移テスト)
---

## 実装上の注意事項

### セキュリティ
- `spawnSync` を使用したシェルインジェクション対策を維持する
- UUID バリデーションを維持する
- `.gitignore` 除外処理を維持する

### パフォーマンス
- `git add .` が大規模リポジトリで遅延しないことを確認する
- 必要に応じて、タイムアウト処理を追加する（現在は実装されていない）

### 互換性
- 既存のイベント駆動アーキテクチャ（`spec.phase_changed`）を維持する
- `handlePhaseChangeCommit()`, `handleSpecCreatedCommit()` のシグネチャ変更不可

### テストカバレッジ
- 既存テストがすべて通過すること
- 全体カバレッジ 80% 以上を維持すること
