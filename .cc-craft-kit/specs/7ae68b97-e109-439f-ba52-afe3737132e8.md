---
id: "7ae68b97-e109-439f-ba52-afe3737132e8"
name: "データベース廃止前の仕様書情報が移行できていない"
phase: "completed"
branch_name: "fix/spec-7ae68b97-migrate-legacy-spec-data"
github_issue_number: null
pr_url: "https://github.com/B16B1RD/cc-craft-kit/pull/904"
created_at: "2025-12-05T23:45:30.000Z"
updated_at: "2025-12-06T00:51:30.000Z"
---
...
- [x] `/cft:status` で全ての仕様書が件数に反映される
- [x] 旧 DB の 187 件の completed 仕様書が JSON に移行される

### 機能要件

- [x] `.cc-craft-kit/specs/` 内の全 .md ファイルが `specs.json` に登録される
- [x] 仕様書のフェーズ情報（特に completed）が正しく移行される
- [x] `/cft:status` のブランチフィルタリングを削除または改善する
...
- [x] `src/core/storage/helpers.ts` に `includeAllBranches` オプションを追加 (#898)
- [x] `getSpecsWithGitHubInfo()` のフィルタリングロジックを修正 (#899)

### Phase 2: /cft:status の修正

- [x] `src/commands/status/get-status.ts` で `includeAllBranches: true` を使用するよう修正 (#900)

### Phase 3: .md ファイル一括移行

- [x] `src/scripts/migrate-specs-from-files.ts` 移行スクリプトを作成 (#901)
- [x] SyncService.importFromDirectory() を呼び出して 189 件の .md ファイルを移行 (#902)
- [x] 移行後の整合性チェック（specs.json の件数確認） (#903)

---

## Pull Request

- https://github.com/B16B1RD/cc-craft-kit/pull/904
