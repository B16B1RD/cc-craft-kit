---
id: "f30e11de-8a4c-4e5d-a006-3fa915eaf6a6"
name: "implementation フェーズ時のタスク進行時、Sub Issue が更新されない"
phase: "completed"
branch_name: "develop"
github_issue_number: null
pr_url: null
created_at: "2025-12-01T01:33:20.097Z"
updated_at: "2025-12-01T12:15:30Z"
---
...
- [x] `getSubIssueInfo()` で `parent_spec_id` を取得して返す
- [x] `executeTaskDone()` で `getEventBusAsync()` を使用
- [x] イベント発火時に正しい `specId` を設定

### Phase 2: テスト追加

- [x] `getSubIssueInfo()` のユニットテスト追加
- [x] 統合テストで Sub Issue 更新を検証

### Phase 3: 動作検証

- [x] `npm run sync:dogfood` で同期
- [x] 実際のタスク完了で Sub Issue が更新されることを確認

### Phase 4: implementation フェーズでの自動更新対応

- [x] `src/commands/task/start.ts` 新規作成（`task.started` イベント発火）
- [x] `src/core/workflow/event-bus.ts` に `task.started` イベントタイプ追加
- [x] `src/core/workflow/github-integration.ts` に `task.started` ハンドラー追加
- [x] `src/slash-commands/spec-phase.md` の Step 8.6 を修正（タスク開始/完了時に Sub Issue 自動更新）
---

## Pull Request

- https://github.com/B16B1RD/cc-craft-kit/pull/492
