---
id: "43affc54-45e2-44a8-a275-195c783f70c8"
name: "GitHub Sub Issue の連携ができていない"
phase: "completed"
branch_name: "develop"
github_issue_number: null
pr_url: null
created_at: "2025-11-30T00:59:27.530Z"
updated_at: "2025-11-30T20:34:00Z"
---
...
- [x] `github_sync` テーブルに `parent_issue_number` カラムを追加するマイグレーション作成
- [x] `github_sync` テーブルに `parent_spec_id` カラムを追加するマイグレーション作成
- [x] `src/core/database/schema.ts` の型定義を更新
- [x] マイグレーション適用と単体テスト作成

### Phase 2: Sub Issue 作成時の親 Issue 関連性記録

- [x] `recordSubIssueSyncData()` メソッドに `options` パラメータを追加
- [x] `create-sub-issues.ts` から親 Issue 番号と仕様書 ID を渡すよう修正
- [x] `createSubIssuesFromTaskList()` の引数を拡張
- [x] 単体テスト更新と検証

### Phase 3: タスク完了時の親 Issue 連携

- [x] `syncParentIssueCheckbox()` メソッドを実装
- [x] `checkAllSubIssuesClosed()` メソッドを実装
- [x] `closeParentIssue()` メソッドを実装
- [x] `task.completed` ハンドラーを拡張して親 Issue 連携を追加
- [x] 統合テスト作成と検証
---

## Pull Request

- https://github.com/B16B1RD/cc-craft-kit/pull/432
