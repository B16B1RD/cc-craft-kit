---
id: "bf2faf6a-3c63-47d6-8d49-35e242135878"
name: "/cft:spec-phase \<id\> design 完了時、仕様書のコミットがされていない"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-22T07:10:01.355Z"
updated_at: "2025-11-22T17:21:36Z"
---
...
1. **各フェーズ移行時のコミット動作テスト**:
   - requirements フェーズでのコミットメッセージ検証
   - design フェーズでのコミットメッセージ検証
   - tasks フェーズでのコミットメッセージ検証
   - implementation フェーズでのコミットメッセージ検証
   - completed フェーズでのコミットメッセージ検証

2. **コミットメッセージ生成ロジックの検証**:
   - `generateCommitMessage()` の各フェーズに対するテスト

3. **エラーハンドリングのテスト**:
   - pre-commit フック失敗時のロールバック動作テスト
   - Git リポジトリ未初期化時の警告表示テスト
   - .gitignore 除外ファイルのスキップテスト

**受け入れ基準:**

- [ ] 全テストケースが通過すること
- [ ] テストカバレッジが 80%以上であること
- [ ] モック化により Git コマンドが実際に実行されないこと
---

### タスク 2: E2E テストの追加

**対象ファイル:** `tests/e2e/phase-transition-commit.test.ts`（新規作成）

**実装内容:**

1. **フェーズ移行→自動コミット→GitHub Issue 更新の一連の流れを検証**:
   - テスト用一時ディレクトリの作成
   - Git リポジトリの初期化
   - 仕様書作成
   - 各フェーズへの移行とコミット検証
...
3. **GitHub Issue 更新の検証**:
   - GitHub API のモック化
   - Issue コメントの追加を検証

**受け入れ基準:**

- [ ] requirements → design → tasks → implementation → completed の全フェーズ移行で自動コミットが成功すること
- [ ] コミットメッセージがフェーズに応じた適切な内容であること
- [ ] GitHub API 呼び出しが正しく行われること
- [ ] テスト終了後、一時ディレクトリが削除されること
---
...
2. **スキップ条件の詳細**:
   - 未コミット変更がない場合
   - .gitignore で除外されているファイルの場合
   - Git リポジトリ未初期化の場合

3. **トラブルシューティング**:
   - pre-commit フック失敗時の対処法
   - textlint/markdownlint エラーの修正方法
   - 手動コミットの手順

4. **FAQ の追加**:
   - 「自動コミットが実行されない」
   - 「pre-commit フックでエラーが発生する」
   - 「コミットメッセージをカスタマイズしたい」

**受け入れ基準:**

- [ ] ドキュメントが textlint/markdownlint でエラーなく通過すること
- [ ] スキップ条件がすべて記載されていること
- [ ] トラブルシューティングガイドが具体的であること
- [ ] FAQ が 3 つ以上記載されていること
---
...
| タスク | 優先度 | 依存関係 | 推定工数 |
|---|---|---|---|
| タスク 1: 単体テストの拡充 | 高 | なし | 2-3 時間 |
| タスク 2: E2E テストの追加 | 高 | タスク 1 完了後 | 3-4 時間 |
| タスク 3: ドキュメント更新 | 中 | なし | 1-2 時間 |

**推奨実装順序:**

1. タスク 1（単体テスト）→ タスク 2（E2E テスト）→ タスク 3（ドキュメント）
2. または、タスク 1 とタスク 3 を並行実施 → タスク 2
---
