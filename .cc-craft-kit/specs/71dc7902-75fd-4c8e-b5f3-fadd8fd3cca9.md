---
id: "71dc7902-75fd-4c8e-b5f3-fadd8fd3cca9"
name: "spec-phase completed 時に PR が自動作成されない（失敗する）"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-21T08:04:17Z"
updated_at: "2025-11-21T10:53:14Z"
---
...
- [ ] **タスク 1**: PR 作成スキルのディレクトリ構造を作成
  - `.claude/skills/pr-creator/` ディレクトリを作成
  - 必要に応じて `templates/` サブディレクトリを作成

- [ ] **タスク 2**: SKILL.md ファイルを作成（基本構造のみ）
  - スキルの概要とトリガー条件を定義
  - 必要なツール（Read, Bash）を明記
  - 実行フローの概要を記述

### Phase 2: PR 生成機能

- [ ] **タスク 3**: PR タイトル生成ロジックを実装
  - 仕様書名を取得
  - フォーマット: `feat: <仕様書名> を実装完了`
  - 最大 256 文字の制約チェック

- [ ] **タスク 4**: PR 本文生成ロジックを実装
  - 概要セクション: 仕様書の「背景と目的」を要約
  - 変更内容セクション: 実装した機能の箇条書き
  - テスト計画セクション: テスト項目のチェックリスト
  - 最大 65536 文字の制約チェック

### Phase 3: 品質チェック機能

- [ ] **タスク 5**: textlint チェック機能を実装
  - `npm run textlint` で一時ファイルをチェック
  - 警告・エラー0 件を確認
  - エラー時は詳細なメッセージを表示

- [ ] **タスク 6**: markdownlint チェック機能を実装
  - `npx markdownlint-cli2` で一時ファイルをチェック
  - 警告・エラー0 件を確認
  - エラー時は詳細なメッセージを表示

### Phase 4: PR 作成実行

- [ ] **タスク 7**: GitHub CLI (gh pr create) 実行ロジックを実装
  - ベースブランチ取得（環境変数 `DEFAULT_BASE_BRANCH`、デフォルト: `develop`）
  - `gh pr create --title "<title>" --body "<body>" --base <branch>` を実行
  - PR URL を取得
  - 仕様書ファイルに PR URL を記録

### Phase 5: イベント統合

- [ ] **タスク 8**: イベントハンドラーに PR 作成スキル呼び出しを追加
  - `src/core/workflow/git-integration.ts` を編集
  - `spec.phase_changed` イベントで `newPhase === 'completed'` の場合に Skill ツールで `pr-creator` を実行
  - エラー時もフェーズ移行は成功させる
