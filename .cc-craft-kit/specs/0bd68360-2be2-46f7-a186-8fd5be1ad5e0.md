...
- [ ] `syncParentIssueCheckbox()` 内のチェックボックスパターンを厳密化（sub-issues.ts:401）
- [ ] チェックボックスパターンの単体テストを追加

### Phase 2: 未使用メソッドの削除

- [ ] `checkAllSubIssuesClosed()` メソッドを削除（sub-issues.ts:447-479）
- [ ] 削除後に既存テストが通ることを確認
