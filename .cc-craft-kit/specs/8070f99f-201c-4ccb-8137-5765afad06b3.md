---
id: "8070f99f-201c-4ccb-8137-5765afad06b3"
name: "/cft:pr-cleanup がまともに動作しない"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-23T10:42:10.577Z"
updated_at: "2025-11-23T20:31:19Z"
---
...
実装は以下のタスクに分解され、順次進めていきます。

#### 優先度 1: コア機能の修正

- [ ] **GitHub クライアント自動初期化処理を実装**
  - `pull-request.ts:119-134` の実装パターンを参考に、未初期化時のフォールバック処理を追加
  - 環境変数 `GITHUB_TOKEN` が存在すれば `initGitHubClient()` を呼び出す
  - 依存: なし

- [ ] **設定読み込みを環境変数と config.json の両方に対応**
  - 環境変数（`GITHUB_OWNER`, `GITHUB_REPO`）を優先的に使用
  - 環境変数が未設定の場合、`.cc-craft-kit/config.json` から読み込む
  - `spec/delete.ts:42-65` の実装パターンを参考にする
  - 依存: なし

- [ ] **PR マージ状態の確認とエラーハンドリング**
  - `client.rest.pulls.get()` で PR 情報を取得
  - `response.data.merged` が `false` の場合、警告メッセージを表示して処理を中断
  - `response.data.base.ref` からベースブランチ名を取得
  - 依存: タスク 1, 2

- [ ] **データベーストランザクション内で branch_name と pr_merged_at を更新**
  - `db.transaction()` を使用してトランザクション内で処理
  - `specs.branch_name` を PR のベースブランチに更新
  - `github_sync.pr_merged_at` にマージ日時を記録
  - 一部失敗時はロールバック
  - 依存: タスク 3

#### 優先度 2: 追加機能

- [ ] **spec.pr_merged イベントの発火処理を追加**
  - `getEventBusAsync()` で EventBus インスタンスを取得
  - `spec.pr_merged` イベントを発火（イベントペイロード: `{ specId, prNumber, mergedAt }`）
  - イベント発火失敗時は警告のみ（処理は継続）
  - 依存: タスク 4

- [ ] **エラーメッセージの改善（具体的な解決策を提示）**
  - GitHub クライアント初期化失敗時: 「Run `/cft:github-init` or set GITHUB_TOKEN.」
  - PR 未マージ時: 「PR #XXX is not merged yet.」
  - GitHub API エラー時: 具体的なエラー内容とユーザーアクションを提示
  - 依存: タスク 1-5

#### 優先度 3: 品質保証

- [ ] **単体テストを作成（80% 以上のカバレッジ）**
  - `tests/integrations/github/pr-cleanup.test.ts` を作成
  - GitHub API 呼び出しはモック化
  - Git 操作はモック化
  - データベースは `:memory:` モードでテスト
  - 依存: タスク 1-6

- [ ] **npm run sync:dogfood でソースコードを同期**
  - `src/` 配下のコード変更を `.cc-craft-kit/` に反映
  - 依存: タスク 1-7
