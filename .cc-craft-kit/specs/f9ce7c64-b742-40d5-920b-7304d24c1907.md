...
- [ ] `src/slash-commands/init.md` から `argument-hint` 行を削除
- [ ] `src/commands/utils/error-handler.ts` のエラーメッセージを `/cft:init` に変更
- [ ] `README.md` の使用例を `/cft:init` に更新
- [ ] `docs/QUICK_START.md` の使用例を `/cft:init` に更新

### Phase 2: テスト更新

- [ ] `tests/integration/commands-cli.integration.ts` を引数なしでテスト実行するよう更新
