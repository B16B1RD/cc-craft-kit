...
- [ ] `src/core/workflow/github-integration.ts` の `spec.created` ハンドラーで GitHub トークン未設定時に警告出力を追加
- [ ] `src/core/workflow/github-integration.ts` の `spec.created` ハンドラーで GitHub 設定未設定時に警告出力を追加
- [ ] 既存テスト（`tests/core/workflow/github-integration.test.ts`）への影響確認・修正

### Phase 2: spec-create.md の修正

- [ ] `src/slash-commands/spec-create.md` の Step 4 でブランチ命名規則を `feature/` に統一
- [ ] `src/slash-commands/spec-create.md` の Step 4 で develop 以外からの実行時に警告を追加
- [ ] `src/slash-commands/spec-create.md` に Step 9.5（仕様書ファイルの自動コミット）を追加
- [ ] `npm run sync:dogfood` で `.cc-craft-kit/` へ同期

### Phase 3: spec-phase.md の修正

- [ ] `src/slash-commands/spec-phase.md` の Step 4 に GitHub Issue 連携状態確認ロジックを追加
- [ ] `npm run sync:dogfood` で `.cc-craft-kit/` へ同期
