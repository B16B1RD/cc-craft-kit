---
id: "de53d0d8-2ff7-4e12-86c6-ec0dd09cc908"
name: "/cft:spec-delete で GitHub Issue のクローズはオプションなしでできるようにしたい"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-23T06:35:13.424Z"
updated_at: "2025-11-23T15:50:09Z"
---
...
- [ ] **タスク 1**: `src/commands/spec/delete.ts` のデフォルト値を `false` から `true` に変更
  - ファイル: `src/commands/spec/delete.ts:64`
  - 内容: `closeGitHubIssue = false` → `closeGitHubIssue = true`
  - 影響範囲: デフォルト動作のみ、既存の `--close-github-issue` オプションは後方互換性を維持
  - 優先度: 高

### 8.2. ドキュメント更新

- [ ] **タスク 2**: `src/slash-commands/spec-delete.md` のドキュメントを更新
...
- [ ] **タスク 3**: `tests/commands/spec/delete.test.ts` にテストケースを追加
  - ファイル: `tests/commands/spec/delete.test.ts`
  - 内容:
    - デフォルト動作のテストケース追加（オプションなしで GitHub Issue がクローズされること）
    - 後方互換性のテストケース追加（`--close-github-issue` 指定時も正常に動作すること）
  - 依存関係: タスク 1 完了後
  - 優先度: 中

### 8.4. 動作確認

- [ ] **タスク 4**: `npm run sync:dogfood` でソースコード同期
  - コマンド: `npm run sync:dogfood`
  - 内容: `src/` の変更を `.cc-craft-kit/` へ同期
  - 依存関係: タスク 1-3 完了後
  - 優先度: 高

- [ ] **タスク 5**: 実際の仕様書削除で GitHub Issue がクローズされることを確認
  - 内容:
    - テスト仕様書を作成
    - GitHub Issue を作成
    - `/cft:spec-delete` でオプションなしで削除
    - GitHub Issue がクローズされていることを確認
  - 依存関係: タスク 4 完了後
  - 優先度: 高
