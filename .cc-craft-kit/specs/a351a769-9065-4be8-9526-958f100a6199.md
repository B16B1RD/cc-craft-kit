---
id: "a351a769-9065-4be8-9526-958f100a6199"
name: "フェーズ移行時に不要なブランチが複数作成される問題を修正"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-21T13:57:27Z"
updated_at: "2025-11-21T20:11:13Z"
---
...
#### タスク 1: `handleBranchCreationOnImplementation` 関数の削除

- **ファイル**: `src/core/workflow/branch-management.ts`
- **対象行**: 149-242
- **内容**: フェーズ移行時にブランチを自動作成する関数を削除
- **理由**: 仕様書作成時に既にブランチが作成されるため、フェーズ移行時の再作成は不要

#### タスク 2: イベントハンドラー登録の削除

- **ファイル**: `src/core/workflow/branch-management.ts`
- **対象行**: 391-397
- **内容**: `spec.phase_changed` イベントから `handleBranchCreationOnImplementation` へのハンドラー登録を削除
- **理由**: タスク 1 で関数を削除するため、ハンドラー登録も不要

#### タスク 3: `handleProtectedBranchWarning` 関数の必要性確認

- **ファイル**: `src/core/workflow/branch-management.ts`
- **対象行**: 245-310
- **内容**: 保護ブランチでの作業時に警告を表示する関数の必要性を確認
- **判断基準**:
  - 仕様書作成時に保護ブランチから自動的に feature ブランチが作成されるため、警告は不要の可能性
  - ただし、手動でブランチを削除して保護ブランチに戻った場合の救済措置として残す価値があるか検討
- **結論**: 必要性が低いと判断した場合は削除、必要なら残す

#### タスク 4: 単体テストの作成

- **ファイル**: `tests/core/workflow/branch-management.test.ts`（新規作成）
- **内容**:
  - 仕様書作成時に 1 つのブランチのみが作成されること
  - フェーズ移行（tasks → implementation）時にブランチが作成されないこと
- **使用ライブラリ**: Jest, in-memory SQLite

#### タスク 5: E2E テストの作成

- **ファイル**: `tests/e2e/branch-creation.test.ts`（新規作成）
- **内容**:
  - 仕様書作成 → フェーズ移行（requirements → design → tasks → implementation）のフルフロー
  - 各ステップでブランチ数が 1 つのみであることを検証
- **使用ライブラリ**: Jest, 実際の Git コマンド

#### タスク 6: ドキュメント更新

- **ファイル**: `CLAUDE.md`
- **対象セクション**:「ブランチ管理」（現在行 138-255）
- **内容**:
  - フェーズ移行時のブランチ作成が削除されたことを明記
  - ブランチ作成のタイミングが仕様書作成時のみであることを強調
  - `handleBranchCreationOnImplementation` の説明を削除
...
1. タスク 1: `handleBranchCreationOnImplementation` 削除
2. タスク 2: イベントハンドラー登録削除
3. タスク 3: `handleProtectedBranchWarning` 必要性確認
4. タスク 4: 単体テスト作成
5. タスク 5: E2E テスト作成
6. タスク 6: ドキュメント更新
7. タスク 7: 統合テスト実行
