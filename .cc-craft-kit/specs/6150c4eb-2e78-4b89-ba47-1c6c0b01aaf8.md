---
id: "6150c4eb-2e78-4b89-ba47-1c6c0b01aaf8"
name: "specs.github_issue_id カラムを削除し、github_sync テーブルに一元化"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-20T12:35:56Z"
updated_at: "2025-11-20T15:24:09Z"
---
...
- [ ] **タスク1: 整合性チェックスクリプトを作成**
  - ファイル: `.cc-craft-kit/scripts/check-github-sync-integrity.ts`
  - 説明: `specs.github_issue_id` と `github_sync.github_number` の整合性を 3 パターンでチェック
  - 優先度: 高
  - 依存: なし

### フェーズ2: データベース層の変更

- [ ] **タスク2: マイグレーションファイル 004_remove_specs_github_issue_id.ts を作成**
  - ファイル: `src/core/database/migrations/004_remove_specs_github_issue_id.ts`
  - 説明: `specs` テーブルから GitHub 関連カラムを削除（Up/Down 両方）
  - 優先度: 高
  - 依存: タスク 1

- [ ] **タスク3: src/core/database/schema.ts の SpecsTable から GitHub 関連カラムを削除**
  - ファイル: `src/core/database/schema.ts`
  - 説明: `github_issue_id`, `github_project_id`, `github_project_item_id`, `github_milestone_id` を削除
  - 優先度: 高
  - 依存: タスク 2

- [ ] **タスク4: src/core/database/helpers.ts にヘルパー関数 getSpecWithGitHubInfo を作成**
  - ファイル: `src/core/database/helpers.ts`（新規作成）
  - 説明: `specs` と `github_sync` を JOIN して GitHub 情報を取得するヘルパー関数
  - 優先度: 高
  - 依存: タスク 3

### フェーズ3: コマンド層の変更

- [ ] **タスク5: src/commands/spec/get.ts を修正して github_sync との JOIN を使用**
  - ファイル: `src/commands/spec/get.ts`
  - 説明: `getSpecWithGitHubInfo()` を使用して GitHub Issue 番号を表示
  - 優先度: 高
  - 依存: タスク 4

- [ ] **タスク6: src/commands/spec/list.ts を修正して github_sync との JOIN を使用**
  - ファイル: `src/commands/spec/list.ts`
  - 説明: 仕様書一覧で GitHub Issue 番号を正しく表示
  - 優先度: 高
  - 依存: タスク 4

- [ ] **タスク7: src/commands/status.ts を修正して github_sync との JOIN を使用**
  - ファイル: `src/commands/status.ts`
  - 説明: プロジェクト状態表示で GitHub 連携状態を正しく表示
  - 優先度: 中
  - 依存: タスク 4

- [ ] **タスク8: src/commands/github/issue-create.ts を修正**
  - ファイル: `src/commands/github/issue-create.ts`
  - 説明: `specs.github_issue_id` への書き込みを削除し、`github_sync` のみ更新
  - 優先度: 高
  - 依存: タスク 3

### フェーズ4: 統合層の変更

- [ ] **タスク9: src/integrations/github/ensure-issue.ts を修正**
  - ファイル: `src/integrations/github/ensure-issue.ts`
  - 説明: GitHub Issue 自動リカバリーロジックを `github_sync` ベースに変更
  - 優先度: 高
  - 依存: タスク 4

- [ ] **タスク10: src/core/workflow/phase-automation-registration.ts を修正**
  - ファイル: `src/core/workflow/phase-automation-registration.ts`
  - 説明: フェーズ変更時の GitHub Issue 更新処理を修正
  - 優先度: 中
  - 依存: タスク 4

### フェーズ5: テスト層の変更

- [ ] **タスク11: テストファイル（10件）を修正**
  - ファイル:
    - `tests/e2e/project-initialization.test.ts`
    - `tests/integrations/github/ensure-issue.test.ts`
    - `tests/commands/spec/phase.test.ts`
    - `tests/core/workflow/github-integration.test.ts`
    - `tests/core/sync/sync-service.test.ts`
    - `tests/core/sync/integrity-checker.test.ts`
    - `tests/integrations/sub-issue-workflow.test.ts`
    - `tests/integrations/github/knowledge-base.test.ts`
    - `tests/integrations/github/sync.test.ts`
    - `tests/core/filesystem/watcher.test.ts`
  - 説明: `specs.github_issue_id` への参照を削除し、`github_sync` ベースに変更
  - 優先度: 中
  - 依存: タスク 1-10

### フェーズ6: 検証とデプロイ

- [ ] **タスク12: 整合性チェックを実行して不整合がないことを確認**
  - コマンド: `npx tsx .cc-craft-kit/scripts/check-github-sync-integrity.ts`
  - 説明: 現在のデータベースに不整合がないことを確認
  - 優先度: 高
  - 依存: タスク 1

- [ ] **タスク13: マイグレーションを実行**
  - コマンド: `npm run db:migrate`
  - 説明: 開発環境でマイグレーションを実行し、成功を確認
  - 優先度: 高
  - 依存: タスク 2, タスク 12

- [ ] **タスク14: E2E テストを実行して動作確認**
  - コマンド: `npm test`
  - 説明: すべてのテストが成功することを確認
  - 優先度: 高
  - 依存: タスク 13

- [ ] **タスク15: CLAUDE.md と docs/ARCHITECTURE.md のドキュメント更新**
  - ファイル: `CLAUDE.md`, `docs/ARCHITECTURE.md`
  - 説明: データベーススキーマセクションとデータモデル図を更新
  - 優先度: 低
  - 依存: タスク 14

### 実装順序

1. **準備**: タスク 1（整合性チェックスクリプト）
2. **データベース**: タスク 2-4（マイグレーション、スキーマ、ヘルパー関数）
3. **コード修正**: タスク 5-10（コマンド層、統合層）
4. **テスト**: タスク 11（テストファイル修正）
5. **検証**: タスク 12-14（整合性チェック、マイグレーション、E2E テスト）
6. **ドキュメント**: タスク 15（ドキュメント更新）
