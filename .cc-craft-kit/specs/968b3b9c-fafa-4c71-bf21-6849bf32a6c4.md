...
- [ ] status.ts に getCurrentBranch をインポート追加 (#740)
- [ ] status.ts の getSpecsWithGitHubInfo() に branchName オプションを追加 (#741)

### Phase 2: get-status.ts の修正

- [ ] get-status.ts に getCurrentBranch をインポート追加 (#742)
- [ ] get-status.ts の getSpecsWithGitHubInfo() に branchName オプションを追加 (#743)

### Phase 3: テスト・検証

- [ ] 型チェック・リント実行で問題がないことを確認 (#744)
- [ ] 手動テスト: /cft:status が別ブランチの仕様書でエラーにならないことを確認 (#745)
