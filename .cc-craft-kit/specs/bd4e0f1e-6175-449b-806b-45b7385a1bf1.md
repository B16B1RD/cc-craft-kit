...
- [ ] GitHubStatusConfig インターフェースと DEFAULT_STATUS_CONFIG を定義 (#564)
- [ ] loadStatusConfig() 関数を実装（config.json から読み込み） (#565)
- [ ] validateStatusConfig() 関数を実装（設定値のバリデーション） (#566)
- [ ] ユニットテストを作成 (#567)

### Phase 2: DynamicStatusMapper 実装

- [ ] DynamicStatusMapper クラスを実装 (#568)
- [ ] 既存の mapPhaseToStatus() を後方互換として維持しつつ新クラスへ移行 (#569)
- [ ] isValidStatus() 型ガード関数を拡張 (#570)
- [ ] DynamicStatusMapper のユニットテストを追加 (#571)

### Phase 3: projects.ts のパラメータ化

- [ ] updateProjectStatus() に statusFieldName パラメータを追加 (#572)
- [ ] detectAvailableStatuses() メソッドを追加 (#573)
- [ ] フォールバック処理を実装（In Review → In Progress） (#574)
- [ ] 既存テストを更新・新規テスト追加 (#575)

### Phase 4: GitHub 初期化コマンド拡張

- [ ] init.ts でステータス自動検出ロジックを追加 (#576)
- [ ] 検出したステータスを config.json に保存 (#577)
- [ ] 初期化時のユーザーガイダンスを更新 (#578)

### Phase 5: 同期サービス統合

- [ ] sync.ts を DynamicStatusMapper 使用に更新 (#579)
- [ ] 統合テストを実施 (#580)
- [ ] ドキュメント更新（README、設定例） (#581)
