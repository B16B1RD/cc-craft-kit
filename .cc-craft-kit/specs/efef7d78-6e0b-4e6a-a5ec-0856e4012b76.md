---
id: "efef7d78-6e0b-4e6a-a5ec-0856e4012b76"
name: "各フェーズ完了時未コミット状態のものがある（特に仕様書）"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-22T12:55:59.805Z"
updated_at: "2025-11-23T11:07:10Z"
---
...
- [ ] **git-integration.ts の getCommitTargets() 関数を修正**
  - ファイル: `src/core/workflow/git-integration.ts:167-184`
  - 内容: すべてのフェーズで `['.']` を返すようにシンプル化
  - 削除: `completed` フェーズのみ全変更をコミットする分岐
  - 削除: 不要な `path.basename()` 呼び出し

- [ ] **phase-transition-commit.test.ts のテストケースを修正**
  - ファイル: `tests/e2e/phase-transition-commit.test.ts:277, 286, 303, 335`
  - 内容: アサーションを `expectCommitTargetsToBe(['.'])` に変更
  - 内容: テストケース名を「全変更ファイルをコミット」に変更
...
- [ ] **動作確認**
  - 内容: 実際にフェーズ移行を実行し、全変更がコミットされることを確認
  - コマンド: `git log` でコミットメッセージが正しいことを確認
---
