---
id: "eba108ca-3ba8-4418-ba80-c0ca6fed22f3"
name: "各フェーズ完了時に未コミットのものがる"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-24T07:03:32.318Z"
updated_at: "2025-11-24T18:25:59Z"
---
...
#### Task 1.1: バリデーションロジックの修正

**ファイル**: `src/core/validators/placeholder-detector.ts`

**目的**: 設計詳細セクションのサブセクション番号が可変である場合に対応

**実装内容**:
- [x] セクション番号を正規表現でマッチング（`/### 7\.\d+\. アーキテクチャ設計/` など）
- [x] 柔軟なバリデーションロジックに変更

**完了条件**:
- バリデーターが異なるセクション番号でも正しく動作する
- 既存のテストがパスする

**優先度**: 最高（すでに完了）
---

#### Task 1.2: spec-phase.md での git status チェック強化

**ファイル**: `src/slash-commands/spec-phase.md`

**目的**: フェーズ遷移後の未コミット状態を確実に検出

**実装内容**:
- [ ] フェーズ遷移後に `git status --porcelain` を自動実行
- [ ] 未コミットファイルがある場合の警告メッセージ表示を明確化
- [ ] ユーザーへの手動コミット案内を改善

**完了条件**:
- フェーズ遷移後に必ず git status チェックが実行される
- 未コミット状態が検出された場合、明確な警告が表示される
...
#### Task 2.1: event-bus.ts のタイムアウト処理追加

**ファイル**: `src/core/workflow/event-bus.ts`

**目的**: ハンドラー登録待機処理がデッドロックしないよう保証

**実装内容**:
- [ ] `getEventBusAsync()` にタイムアウトパラメータを追加（デフォルト: 10秒）
- [ ] タイムアウト時のエラーメッセージを明確化
- [ ] ログレベルを `error` に設定

**完了条件**:
- タイムアウト処理が正常に動作する
- タイムアウト時に明確なエラーメッセージが表示される
...
#### Task 2.2: phase.ts のエラーハンドリング強化
...
**実装内容**:
- [ ] ハンドラー登録待機のタイムアウトエラーをキャッチ
- [ ] ロールバック処理の改善（DB、ファイル、Git ブランチ）
- [ ] エラーログの詳細化
...
#### Task 2.3: git-integration.ts のログレベル改善

**ファイル**: `src/core/workflow/git-integration.ts`

**目的**: 自動コミット処理のログ出力を改善

**実装内容**:
- [ ] コミット成功時のログレベルを `info` に設定
- [ ] コミット失敗時のログレベルを `warn` に設定
- [ ] ログメッセージを詳細化
...
#### Task 3.1: event-bus.test.ts にハンドラー登録待機のテストを追加

**ファイル**: `tests/core/workflow/event-bus.test.ts`

**目的**: `getEventBusAsync()` の動作を検証
...
#### Task 3.2: phase.test.ts にフェーズ遷移時の自動コミットテストを追加

**ファイル**: `tests/commands/spec/phase.test.ts`

**目的**: フェーズ遷移時の自動コミット処理を検証

**実装内容**:
- [ ] `spec.phase_changed` イベント発火時の自動コミットテスト
- [ ] コミットメッセージ生成のテスト
- [ ] Git リポジトリが存在しない場合のスキップテスト
- [ ] エラーハンドリングのテスト
...
#### Task 3.3: phase-commit-flow.test.ts に統合テストを追加

**ファイル**: `tests/integrations/phase-commit-flow.test.ts`

**目的**: E2E フローで自動コミット処理を検証

**実装内容**:
- [ ] フェーズ遷移 → 自動コミット → GitHub Issue 更新のフロー
- [ ] 未コミット状態の検出テスト
- [ ] エラーハンドリングのテスト
...
1. **Task 1.2**: spec-phase.md の git status チェック強化（最優先）
2. **Task 2.1**: event-bus.ts のタイムアウト処理追加
3. **Task 2.2**: phase.ts のエラーハンドリング強化
4. **Task 2.3**: git-integration.ts のログレベル改善
5. **Task 3.1**: event-bus.test.ts のテスト追加
6. **Task 3.2**: phase.test.ts のテスト追加
7. **Task 3.3**: phase-commit-flow.test.ts の統合テスト追加
8. **Task 4.1**: カバレッジ確認
9. **Task 4.2**: 型チェック・リント実行
