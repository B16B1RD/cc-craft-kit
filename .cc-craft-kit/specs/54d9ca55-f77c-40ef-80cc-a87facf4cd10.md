...
1. **段階的インストール方法の提示**:
   - 推奨方法: git clone（最新ソースコードを取得）
   - 代替方法: GitHub Releases（安定版を取得）
   - 将来対応: npm install（現時点では未対応と明記）

2. **既存の curl インストール方法を削除または非推奨化**:
   - 存在しないドメイン `cc-craft-kit.dev` への参照を完全削除
   - curl コマンドは GitHub raw URL 経由で提供可能だが、現時点では git clone を推奨

3. **インストールスクリプトの整合性確保**:
   - scripts/install.sh 内のコメント・ヘルプを更新
   - 実際の実行方法と一致させる
...
| ファイル | 変更内容 |
|---------|---------|
| README.md | インストールセクション（行 29-56）を git clone 推奨に変更 |
| docs/QUICK_START.md | ステップ1（行 11-49）を git clone 推奨に変更 |
| scripts/install.sh | ヘルプ・コメント内の URL を GitHub raw URL または削除 |
...
1. [Releases ページ](https://github.com/B16B1RD/cc-craft-kit/releases) から最新版をダウンロード
2. アーカイブを展開し、プロジェクトディレクトリに配置
3. `npm install` を実行

#### 将来対応: npm パッケージ

npm パッケージとしての公開は将来対応予定です。
```
...
| 機能 | 現在の実装 | 新しい実装 | 実装場所 |
|-----|----------|----------|---------|
| 推奨インストール | curl（存在しないドメイン） | git clone | README.md, docs/QUICK_START.md |
| 代替インストール | 開発者向けクローン | GitHub Releases | README.md, docs/QUICK_START.md |
| スクリプトヘルプ | cc-craft-kit.dev URL | GitHub raw URL または削除 | scripts/install.sh |
...
- [ ] README.md のインストールセクションを git clone 推奨に変更 (#520)
- [ ] docs/QUICK_START.md のインストールセクションを git clone 推奨に変更 (#521)
- [ ] scripts/install.sh のヘルプ・コメント内の URL を修正 (#522)
