---
id: "f524a49a-52d4-4596-aca2-0322a6b882de"
name: "/cft:spec-phase <id> implementation において、実装が自動で実行されない"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-27T10:24:09.295Z"
updated_at: "2025-11-27T13:28:00Z"
---
...
- [x] `src/slash-commands/spec-phase.md` の implementation セクション（行 171-176）を特定
- [x] 現在の 4 行の定義を削除
- [x] 新しい詳細定義（約 60 行）を追加
  - [x] 実装タスクリスト読み込み処理を追加
  - [x] タスク解析・TodoWrite 登録処理を追加
  - [x] 設計詳細確認処理を追加
  - [x] typescript-eslint スキル実行処理を追加
  - [x] エラー有無に応じたガイダンス表示を追加

### Phase 2: 同期と動作確認

- [x] `npm run sync:dogfood` を実行して同期
- [x] `npm run check:sync` で同期状態を確認
- [x] 型チェック（`npm run typecheck`）を実行
- [x] この仕様書で `/cft:spec-phase f524a49a impl` を実行して動作確認
---

## Pull Request

- https://github.com/B16B1RD/cc-craft-kit/pull/401
