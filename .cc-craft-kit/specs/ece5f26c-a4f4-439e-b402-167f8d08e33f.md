...
- [ ] `detectChanges`, `buildChangelogComment` をインポート追加
- [ ] `spec.phase_changed` ハンドラーで既存 Issue 内容を取得
- [ ] `detectChanges()` で変更検出を実装
- [ ] `issues.update()` に `body` パラメータを必ず含める
- [ ] 変更履歴コメント（📝 仕様書更新）追加ロジック実装
- [ ] デバッグログを追加

### Phase 2: テスト

- [ ] 単体テストを追加（`github-integration.test.ts`）
