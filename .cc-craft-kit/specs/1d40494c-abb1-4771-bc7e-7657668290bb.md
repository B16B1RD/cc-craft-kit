---
id: "1d40494c-abb1-4771-bc7e-7657668290bb"
name: "ブランチを作成せずに作業が実施される"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-20T09:57:45Z"
updated_at: "2025-11-20T10:42:53Z"
---
...
- [ ] .env に BASE_BRANCH, GITHUB_DEFAULT_BASE_BRANCH 設定を追加
- [ ] ブランチ名生成ロジックを実装（src/core/utils/branch-name-generator.ts）
  - プレフィックス判定（feature/fix/refactor/docs/chore/hotfix）
  - 仕様書名のスラッグ化（小文字、スペース→ハイフン、特殊文字除去、最大 50 文字）

### フェーズ移行時のブランチ管理

- [ ] tasks → implementation フェーズ移行時のブランチ自動作成を実装
  - develop（または BASE_BRANCH）から作業ブランチを作成
  - 既にブランチが存在する場合はチェックアウト
  - ブランチ作成失敗時のロールバック処理
- [ ] 保護ブランチでの作業時の警告表示を実装
  - フェーズ移行前に保護ブランチをチェック
  - 警告メッセージ表示と推奨ブランチ名の提示

### PR 自動作成機能

- [ ] PR 自動作成機能を実装（src/integrations/github/pull-request.ts）
  - PR タイトル・本文の自動生成（仕様書の内容から生成）
  - GitHub REST API を使用した PR 作成
  - エラーハンドリング（API エラー、レート制限対応）
- [ ] completed フェーズ移行時の PR 自動作成を実装
  - 作業ブランチから develop へ PR 作成
  - PR 作成後、GitHub Issue にリンクを記録
  - PR 作成失敗時は警告のみ表示（フェーズ移行は成功）

### Hotfix ワークフロー

- [ ] Hotfix ワークフローの特別処理を実装
  - 仕様書名から hotfix 判定（「緊急」「Hotfix」「Critical」）
  - main ブランチから hotfix ブランチを作成
  - completed フェーズ移行時に main へ PR 作成
  - バックポート PR の自動作成（または手動作成を案内）

### エラーハンドリングとテスト

- [ ] エラーハンドリングとロールバック処理を実装
  - Git コマンド実行エラーのハンドリング
  - フェーズ移行のロールバック処理
  - ログレベルの適切な使い分け（info/warn/error）
- [ ] 単体テスト実装（branch-name-generator, pull-request）
  - ブランチ名生成ロジックのテスト（各プレフィックス、スラッグ化）
  - PR 作成機能のテスト（GitHub API モック使用）
- [ ] E2E テスト実装（ブランチ作成 → PR 作成 → マージ）
  - tasks → implementation フェーズ移行時のブランチ作成
  - completed フェーズ移行時の PR 作成
  - hotfix ワークフローの検証
