...
- [ ] `src/integrations/github/sync.ts` のテンプレート判定ロジックを削除
- [ ] 常に `buildIssueBody()` で本文を更新するように変更
- [ ] 単体テスト: `sync.ts` の本文更新ロジック

### P1: 変更履歴の要約記録

- [ ] `src/integrations/github/changelog-writer.ts` を新規作成
  - [ ] `parseSections()` - Markdown セクション解析
  - [ ] `detectChanges()` - セクション単位の差分検出
  - [ ] `generateDiffSummary()` - 差分要約生成
  - [ ] `buildChangelogComment()` - コメント本文生成
- [ ] `src/core/workflow/github-integration.ts` の `spec.updated` ハンドラーに統合
- [ ] 単体テスト: `changelog-writer.ts`

### P2: チェックボックス同期（仕様書 → Issue）

- [ ] `src/integrations/github/checkbox-sync.ts` を新規作成
  - [ ] `parseCheckboxes()` - チェックボックス抽出
  - [ ] `CheckboxSyncService.syncToIssue()` - 仕様書 → Issue 同期
- [ ] 既存の Issue 本文同期に統合（追加作業なし、本文全体を更新するため）
- [ ] 単体テスト: `parseCheckboxes()`

### P3: チェックボックス同期（Issue → 仕様書）

- [ ] `CheckboxSyncService.syncToSpec()` を実装
  - [ ] Issue 本文からチェックボックス状態を抽出
  - [ ] 仕様書ファイルを更新
  - [ ] git commit で変更を記録
- [ ] `src/integrations/github/webhook.ts` の `handleIssueEdited` を拡張
- [ ] DB マイグレーション: `checkbox_hash`, `last_body_hash` カラム追加
- [ ] `/cft:github-sync` コマンドを新規作成（プロンプトベース）
- [ ] 単体テスト: `CheckboxSyncService.syncToSpec()`
- [ ] 統合テスト: Webhook フロー

### 共通

- [ ] 全機能の統合テスト
- [ ] ドキュメント更新（README、ARCHITECTURE.md）
