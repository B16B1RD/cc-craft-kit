---
id: "74066f5a-3c0c-47d9-ad8c-9911e40eeadc"
name: "/cft:status でデータベース整合性の警告が表示される"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-21T13:23:27.997Z"
updated_at: "2025-11-21T22:40:39Z"
---
...
- **内容**: `dbRecordMap` を `Map<string, string>` から `Map<string, { id: string, name: string, branch_name: string }>` に変更
- **対象ファイル**: `src/core/validators/database-integrity-checker.ts:183`
- **依存**: なし
- **優先度**: 高

#### 2. ブランチフィルタリングの追加

- **内容**: 204-210 行目に、データベースレコードの `branch_name` を使ったブランチフィルタリングを追加
- **対象ファイル**: `src/core/validators/database-integrity-checker.ts:204-210`
- **依存**: タスク 1 完了後
- **優先度**: 高

#### 3. テストケース追加（Case 1）

- **内容**: feature ブランチから develop ブランチの仕様書を誤検知しないことを確認
- **対象ファイル**: `tests/core/validators/database-integrity-checker.test.ts`
- **依存**: タスク 2 完了後
- **優先度**: 中

#### 4. テストケース追加（Case 2）

- **内容**: develop ブランチから feature ブランチの仕様書を誤検知しないことを確認
- **対象ファイル**: `tests/core/validators/database-integrity-checker.test.ts`
- **依存**: タスク 2 完了後
- **優先度**: 中
...
- **内容**: main と develop ブランチの仕様書は正常に検出されることを確認
- **対象ファイル**: `tests/core/validators/database-integrity-checker.test.ts`
- **依存**: タスク 2 完了後
- **優先度**: 中
...
- **内容**: 真の不整合ファイルはブランチに関係なく検出されることを確認
- **対象ファイル**: `tests/core/validators/database-integrity-checker.test.ts`
- **依存**: タスク 2 完了後
- **優先度**: 中
...
- **内容**: `npm test` で既存のテストがすべてパスすることを確認
- **依存**: タスク 6 完了後
- **優先度**: 高

#### 8. 動作確認

- **内容**: `/cft:status` を実行して警告が出ないことを確認
- **依存**: タスク 7 完了後
- **優先度**: 高
