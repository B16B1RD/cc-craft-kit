...
| # | タスク | 状態 | 備考 |
|---|--------|------|------|
| 1.1 | `src/commands/spec/register.ts` を新規作成 | 🔲 未着手 | DB INSERT + イベント発火のみ、約 50 行 |
| 1.2 | `register.ts` の単体テストを作成 | 🔲 未着手 | `tests/commands/spec/register.test.ts` |

### Phase 2: スラッシュコマンド更新

| # | タスク | 状態 | 備考 |
|---|--------|------|------|
| 2.1 | `src/slash-commands/spec-create.md` を新設計に更新 | 🔲 未着手 | Step 1〜10 の処理フローに整理 |

### Phase 3: 旧コード削除

| # | タスク | 状態 | 備考 |
|---|--------|------|------|
| 3.1 | `src/commands/spec/create.ts` を削除 | 🔲 未着手 | `register.ts` 作成後 |
| 3.2 | `src/core/git/branch-creation.ts` を削除 | 🔲 未着手 | Bash ツールで代替 |
| 3.3 | `src/core/git/branch-cache.ts` を削除 | 🔲 未着手 | 依存関係確認後 |

### 検証

| # | タスク | 状態 | 備考 |
|---|--------|------|------|
| 4.1 | `npm run sync:dogfood` で同期 | 🔲 未着手 | |
| 4.2 | `/cft:spec-create` コマンドをテスト | 🔲 未着手 | 新規仕様書作成で動作確認 |
