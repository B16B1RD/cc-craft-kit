---
id: "d29e6a8c-b249-4f84-9444-f988347ebcb1"
name: "/cft:status が機能していない"
phase: "completed"
branch_name: "fix/spec-d29e6a8c-fix-status-command-display"
github_issue_number: null
pr_url: "https://github.com/B16B1RD/cc-craft-kit/pull/896"
created_at: "2025-12-05T16:30:00.000Z"
updated_at: "2025-12-05T21:30:00.000Z"
---
...
- [x] `/cft:status` 実行時に「Database Schema Validation Report」が表示されない
- [x] 仕様書状況でファイルベースの正確なフェーズ別集計が表示される
- [x] completed フェーズの仕様書数が正しく表示される

### 機能要件

- [x] `status.md` から database-schema-validator スキル呼び出しを削除
- [x] `status.md` の --verbose オプションを JSON ストレージ対応に修正
- [x] 仕様書ファイルからフェーズ情報を読み取る仕組みの実装または修正

### 非機能要件

- [x] 既存の `/cft:status` コマンドの出力フォーマットを維持
- [x] パフォーマンス: 仕様書が 200 件以上でも 5 秒以内に結果を表示
...
- [x] status.md の --verbose セクションから db/info.ts 参照を削除 (#891)
- [x] JSON ストレージ統計情報の表示を追加（ファイルサイズ、エントリ数） (#892)

### Phase 3: 動作確認

- [x] `/cft:status` 実行時に DB 関連出力がないことを確認 (#893)
- [x] `/cft:status --verbose` が正常動作することを確認 (#894)
- [x] 同期実行（`npm run sync:dogfood`） (#895)

---

## Pull Request

- https://github.com/B16B1RD/cc-craft-kit/pull/896
