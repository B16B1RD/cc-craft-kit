---
id: "030af17a-fea5-46f4-a62b-2d4872a5fff6"
name: "各フェーズの実行に際し、既存のサブエージェントとスキルでは不足する場合は、適切なサブエージェントとスキルを自動作成し、適用する仕組みを実現"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-19T11:08:21Z"
updated_at: "2025-11-21T15:25:49Z"
---
...
cc-craft-kit は仕様駆動開発をサポートするため、各フェーズで品質チェックを実施しています。現在、以下のサブエージェントとスキルを利用可能です。
...
- [ ] `/cft:quality-generate <type> <name>` コマンドで手動サブエージェント・スキルの生成ができること
...
- [ ] `/cft:quality-check <spec-id>` コマンドで手動品質チェック不足の検出ができること
...
- Claude Code のサブエージェント・スキル仕様に厳密に準拠すること
- 品質要件定義ファイルは Git 管理対象とし、プロジェクト全体で共有可能であること
- 既存のサブエージェント・スキルとの命名衝突を回避すること
...
- [ ] 品質要件定義ファイルの YAML スキーマを設計する
  - 実装ファイル: `src/core/quality/schema.ts`
  - 参考: YAML Schema Specification
  - 必須フィールド: `version`, `quality_requirements[]`
  - 各要件フィールド: `name`, `type`, `trigger_phase`, `description`, `template`, `tools`, `parameters`

- [ ] テンプレートエンジン拡張インターフェースを実装する
  - 実装ファイル: `src/core/templates/engine.ts`
  - 型定義: `SubagentTemplateVars`, `SkillTemplateVars`
  - メソッド: `renderSubagent()`, `renderSkill()`
  - Handlebars ベースのテンプレートレンダリング

- [ ] サブエージェントテンプレートファイルを作成する
  - ディレクトリ: `.cc-craft-kit/quality-templates/subagents/`
  - テンプレート例: `security-auditor.md`, `api-doc-generator.md`
  - フロントマター構造: 既存 `.claude/agents/*.md` に準拠

- [ ] スキルテンプレートファイルを作成する
  - ディレクトリ: `.cc-craft-kit/quality-templates/skills/`
  - テンプレート例: `api-doc-generator/SKILL.md`
  - フロントマター構造: 既存 `.claude/skills/**/SKILL.md` に準拠

- [ ] `/cft:quality-init` コマンドを実装する
  - 実装ファイル: `src/commands/quality/init.ts`
  - スラッシュコマンド: `src/slash-commands/quality-init.md`
  - 機能: `.cc-craft-kit/quality-requirements.yaml` テンプレート生成
  - バリデーション: 既存サブエージェント・スキル名の衝突チェック

- [ ] `/cft:quality-generate` コマンドを実装する
  - 実装ファイル: `src/commands/quality/generate.ts`
  - スラッシュコマンド: `src/slash-commands/quality-generate.md`
  - 引数: `<type>` (subagent/skill), `<name>`
  - 機能: テンプレートから `.claude/agents/` または `.claude/skills/` へファイル生成
  - バリデーション: フロントマター構造チェック

### フェーズ2: 自動化機能

- [ ] 品質要件とサブエージェント・スキルのマッピングロジックを実装する
  - 実装ファイル: `src/core/workflow/quality-automation.ts`
  - 機能: 品質要件 YAML を読み込み、既存サブエージェント・スキルと比較
  - 出力: 不足している品質チェックのリスト

- [ ] `/cft:quality-check` コマンドを実装する
  - 実装ファイル: `src/commands/quality/check.ts`
  - スラッシュコマンド: `src/slash-commands/quality-check.md`
  - 引数: `<spec-id>`
  - 機能: 手動品質チェック不足検出、結果表示

- [ ] イベントバス統合を実装する
  - 実装ファイル: `src/core/workflow/phase-automation-registration.ts`
  - イベント: `spec.phase_changed`
  - ハンドラー: 品質要件チェック、不足検出、ログ記録
  - トリガーフェーズ: `trigger_phase` に応じて実行

- [ ] 品質チェック履歴のログ記録を実装する
  - 方式 1: `logs.metadata` へ JSON 記録
  - 方式 2: 専用テーブル `quality_checks` 作成（マイグレーション `004_add_quality_checks_table.ts`）
  - 記録内容: `requirement_name`, `agent_or_skill_name`, `type`, `status`, `error_message`

### テスト

- [ ] 単体テストを作成する
  - テストファイル: `tests/core/quality/schema.test.ts`, `tests/core/templates/engine.test.ts`
  - テストケース: YAML パース、テンプレートレンダリング、バリデーション
  - カバレッジ目標: 80%以上

- [ ] E2E テストを作成する
  - テストファイル: `tests/integrations/quality-automation.test.ts`
  - テストシナリオ: フェーズ移行時の自動品質チェック、サブエージェント生成、実行
  - 期待結果: ログ記録、生成されたファイルの存在確認
