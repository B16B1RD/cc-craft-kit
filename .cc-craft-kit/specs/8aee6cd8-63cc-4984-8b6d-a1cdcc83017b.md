---
id: "8aee6cd8-63cc-4984-8b6d-a1cdcc83017b"
name: "/cft:spec-delete で仕様書に対応する GitHub Issue がクローズされない"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-23T07:42:52.537Z"
updated_at: "2025-11-23T17:04:48Z"
---
...
- [ ] **spec.deleted イベントハンドラーの実装** (`src/core/workflow/github-integration.ts`)
  - `spec.deleted` イベントをリッスン
  - `github_sync` テーブルから Project Item ID を取得
  - `GitHubProjects.updateProjectStatus()` で "Done" に更新
  - `verifyProjectStatusUpdate()` で検証
  - エラーハンドリング（404, 401/403, 500）

- [ ] **spec.deleted イベントの発火確認** (`src/commands/spec/delete.ts`)
  - 既存の `spec.deleted` イベント発火処理を確認
  - イベントペイロードに必要なデータが含まれているか検証

### 8.2. テスト実装

- [ ] **単体テスト** (`tests/core/workflow/github-integration.test.ts`)
  - spec.deleted イベントハンドラーのテスト
  - Project ステータス更新のテスト
  - エラーハンドリングのテスト（404, 401/403, 500）
  - github_sync テーブルからのデータ取得テスト

- [ ] **統合テスト** (`tests/integrations/github/sync.test.ts`)
  - `/cft:spec-delete` → spec.deleted イベント → GitHub 連携の E2E テスト
  - GitHub API エラー発生時の動作確認
