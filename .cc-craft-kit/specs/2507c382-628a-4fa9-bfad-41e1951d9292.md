...
- [ ] **重複チェック関数 checkExistingIssue() を実装**
  - ファイル: `src/integrations/github/issue-create.ts`
  - 実装内容: github_sync テーブルをクエリし、既存 Issue の有無を確認
  - 見積もり: 小
  - 依存: なし

- [ ] **createIssueForSpec() にトランザクション制御を追加**
  - ファイル: `src/integrations/github/issue-create.ts`
  - 実装内容: トランザクション内でチェックと作成
  - 見積もり: 中
  - 依存: 重複チェック関数

- [ ] **重複検出時のエラーハンドリングを実装**
  - ファイル: `src/integrations/github/issue-create.ts`
  - 実装内容: DUPLICATE_ISSUE エラーを throw、Issue URL を含むメッセージ
  - 見積もり: 小
  - 依存: トランザクション制御

- [ ] **spec.created イベントハンドラーに重複チェックを適用**
  - ファイル: `src/core/workflow/event-bus.ts`
  - 実装内容: 自動 Issue 作成時に重複チェックを実行
  - 見積もり: 小
  - 依存: エラーハンドリング
