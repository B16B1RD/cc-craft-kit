...
- [ ] Issue タイトルは `【SDD】<仕様書名>` 形式
- [ ] Issue 本文には仕様書の背景・目的が含まれる
- [ ] Issue には `sdd` ラベルが付与される
...
2. **Issue 作成:**
   ```bash
   gh issue create --title "【SDD】$SPEC_NAME" --body "$ISSUE_BODY" --label "sdd"
   ```
   ...
   $SPEC_DESCRIPTION（または仕様書の背景セクションから抽出）
   ...
   - 仕様書ファイル: `.cc-craft-kit/specs/$SPEC_ID.md`
   ```
