---
id: "aab74421-5c39-4280-832e-cb72262a6205"
name: "開発キット全体の処理（設計）見直し"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-23T03:10:44.712Z"
updated_at: "2025-11-23T13:10:36Z"
---
...
### Phase 1: 基準策定・現状調査（優先度: 高）

- [ ] **タスク 1.1**: スクリプト実装とプロンプトベース実装の判断基準を定義
  - 依存関係: なし
  - 説明: データベース操作、GitHub API呼び出し、イベント駆動処理など、スクリプト実装が必須の処理を明確化
  - 成果物: 判断基準チェックリスト（仕様書7.7.1に記載済み）

- [ ] **タスク 1.2**: 現在のスクリプトを調査してプロンプトで代替可能なものを特定
  - 依存関係: タスク 1.1
  - 説明: `src/commands/quality/` 配下のスクリプトを調査し、軽量な処理を特定
  - 成果物: 移行候補リスト（仕様書7.1.3に記載済み）

- [ ] **タスク 1.3**: カスタムスラッシュコマンド → スキル → サブエージェントの処理フロー図を作成
  - 依存関係: なし
  - 説明: 現在の処理フローを図示し、プロンプト層・スクリプト層・コア層の責務を明確化
  - 成果物: 処理フロー図（仕様書7.1.4に記載済み）

### Phase 2: 移行計画策定（優先度: 高）

- [ ] **タスク 2.1**: code-review.ts をプロンプトベースに移行する計画を策定
  - 依存関係: タスク 1.1, 1.2
  - 説明: `code-review.ts` のスクリプトを削除し、`code-review.md` に統合する手順を計画
  - 成果物: 移行手順書

- [ ] **タスク 2.2**: refactor.ts をプロンプトベースに移行する計画を策定
  - 依存関係: タスク 1.1, 1.2
  - 説明: `refactor.ts` のスクリプトを削除し、`refactor.md` に統合する手順を計画
  - 成果物: 移行手順書

- [ ] **タスク 2.3**: test-generate.ts をプロンプトベースに移行する計画を策定
  - 依存関係: タスク 1.1, 1.2
  - 説明: `test-generate.ts` のスクリプトを削除し、`test-generate.md` に統合する手順を計画
  - 成果物: 移行手順書

- [ ] **タスク 2.4**: lint-check.ts をプロンプトベースに移行する計画を策定
  - 依存関係: タスク 1.1, 1.2
  - 説明: `lint-check.ts` を Bash ツールでの `npm run lint` 直接実行に変更する手順を計画
  - 成果物: 移行手順書

- [ ] **タスク 2.5**: schema-validate.ts をプロンプトベースに移行する計画を策定
  - 依存関係: タスク 1.1, 1.2
  - 説明: `schema-validate.ts` のスクリプトを削除し、`schema-validate.md` に統合する手順を計画
  - 成果物: 移行手順書

### Phase 3: ドキュメント更新（優先度: 中）

- [ ] **タスク 3.1**: スクリプト実装を維持すべき処理の範囲を文書化
  - 依存関係: タスク 1.1
  - 説明: データベース層、イベントバス、GitHub API統合などのコア処理を明確化
  - 成果物: 文書（仕様書7.6に記載済み）

- [ ] **タスク 3.2**: CLAUDE.md に設計指針セクションを追加
  - 依存関係: タスク 1.1, 3.1
  - 説明: スクリプトとプロンプトの使い分け指針を CLAUDE.md に追記
  - 成果物: CLAUDE.md の更新（仕様書7.7.1に案を記載済み）

- [ ] **タスク 3.3**: docs/ARCHITECTURE.md にプロンプトベースアーキテクチャセクションを追加
  - 依存関係: タスク 1.3
  - 説明: 3層アーキテクチャ、処理フロー例、設計原則を docs/ARCHITECTURE.md に追記
  - 成果物: docs/ARCHITECTURE.md の更新（仕様書7.7.2に案を記載済み）

### Phase 4: 品質保証（優先度: 中）

- [ ] **タスク 4.1**: 既存の設計パターン（モジュラーモノリス、イベント駆動、DI）の維持を確認
  - 依存関係: すべてのタスク
  - 説明: 移行後もアーキテクチャの一貫性が保たれていることを確認
  - 成果物: レビューレポート
...
| 優先度 | タスクID | 内容 | 期待完了時期 |
|---|---|---|---|
| 高 | 1.1, 1.2, 1.3 | 基準策定・現状調査 | Week 1 |
| 高 | 2.1, 2.2, 2.3, 2.4, 2.5 | 移行計画策定 | Week 2 |
| 中 | 3.1, 3.2, 3.3 | ドキュメント更新 | Week 3 |
| 中 | 4.1 | 品質保証 | Week 4 |

### 注意事項

- **スクリプトに残すべき処理**: データベース層、イベントバス、GitHub API統合、Git操作、ファイル監視は常にスクリプト実装を維持
- **テスト**: 移行後、すべてのスラッシュコマンドが正常に動作することを確認
- **同期**: `src/` を編集したら必ず `npm run sync:dogfood` を実行
