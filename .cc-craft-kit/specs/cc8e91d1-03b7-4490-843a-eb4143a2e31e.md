...
- [ ] **タスク 1**: プレースホルダー検出器の実装(`placeholder-detector.ts`)
  - 仕様書ファイルからプレースホルダーパターンを検出
  - 検出結果を `PlaceholderDetectionResult` 型で返す
  - 想定ファイルパス: `src/core/validators/placeholder-detector.ts`
  - 想定工数: 2〜3時間

- [ ] **タスク 2**: プレースホルダー検出器の単体テスト作成
  - プレースホルダーあり/なし/部分的にあるケースをテスト
  - 想定ファイルパス: `tests/core/validators/placeholder-detector.test.ts`
  - 想定工数: 1〜2時間

- [ ] **タスク 3**: フェーズ遷移前バリデーターの実装(`phase-transition-validator.ts`)
  - フェーズ遷移前に仕様書の品質をチェック
  - プレースホルダー検出器を呼び出してバリデーション
  - 想定ファイルパス: `src/core/workflow/phase-transition-validator.ts`
  - 想定工数: 2〜3時間

- [ ] **タスク 4**: フェーズ遷移前バリデーターの単体テスト作成
  - バリデーション成功/失敗ケースをテスト
  - 想定ファイルパス: `tests/core/workflow/phase-transition-validator.test.ts`
  - 想定工数: 1〜2時間

### Phase 2: 自動補完機能の実装

- [ ] **タスク 5**: 仕様書自動補完サービスの実装(`spec-auto-completer.ts`)
  - Explore サブエージェントでコードベース解析
  - AskUserQuestion ツールでユーザーに質問
  - Edit ツールで仕様書を更新
  - 想定ファイルパス: `src/core/workflow/spec-auto-completer.ts`
  - 想定工数: 4〜6時間

- [ ] **タスク 6**: 仕様書自動補完サービスの単体テスト作成
  - Task ツールと AskUserQuestion のモック化
  - 自動補完成功/失敗ケースをテスト
  - 想定ファイルパス: `tests/core/workflow/spec-auto-completer.test.ts`
  - 想定工数: 2〜3時間

### Phase 3: コマンド統合

- [ ] **タスク 7**: `phase.ts` にフェーズ遷移前バリデーションを統合
  - `updateSpecPhase()` 関数にバリデーション処理を追加
  - フェーズ遷移前に `validatePhaseTransition()` を呼び出し
  - 想定ファイルパス: `src/commands/spec/phase.ts`
  - 想定工数: 2〜3時間

- [ ] **タスク 8**: コマンドラインオプション(`--retry`, `--force`, `--dry-run`)の追加
  - Zod スキーマでオプションを検証
  - 各オプションに応じた処理フローを実装
  - 想定ファイルパス: `src/commands/spec/phase.ts`
  - 想定工数: 2〜3時間
...
- [ ] **タスク 9**: E2Eテストの作成(`phase-transition-auto-complete.test.ts`)
  - Requirements → Design の遷移テスト
  - Design → Implementation の遷移テスト
  - `--force` フラグでのスキップテスト
  - `--retry` フラグでのリトライテスト
  - 想定ファイルパス: `tests/e2e/phase-transition-auto-complete.test.ts`
  - 想定工数: 3〜4時間

- [ ] **タスク 10**: 既存テストの修正とカバレッジ確認
  - 既存のフェーズ遷移テストを修正
  - カバレッジ 80% 以上を確認
  - 想定工数: 1〜2時間
