...
- [ ] `src/integrations/github/phase-status-mapper.ts` の `Phase` 型定義に `'testing'` を追加
- [ ] `mapPhaseToStatus()` のマッピングに `testing: 'In Progress'` を追加
- [ ] `npm run typecheck` で型エラーがないことを確認

### Phase 2: ガイダンス実装

- [ ] `src/commands/spec/phase.ts` の `switch` 文に `case 'testing':` を追加
- [ ] テスト生成・実行、カバレッジ確認、completed への遷移を案内するガイダンスメッセージを実装

### Phase 3: テスト追加

- [ ] `tests/integrations/github/phase-status-mapper.test.ts` に `testing` フェーズの個別テストを追加
- [ ] 統合テストの `phases` 配列に `testing` を追加
- [ ] `npm test` で全テストが通ることを確認

### Phase 4: 動作確認

- [ ] `/cft:spec-phase <id> testing` を実行して正常動作を確認
- [ ] GitHub Project のステータスが `In Progress` に更新されることを確認
- [ ] 適切なガイダンスメッセージが表示されることを確認
