---
id: "ebac31c7-1bc5-44b7-a944-97c0ac06acf3"
name: "仕様書をどのフェーズからでも破棄（クローズ）できるようにしたい"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-23 13:34:24"
updated_at: "2025-11-23T14:31:26Z"
---
...
### タスク 1: GitHub Issue クローズ処理の有効化

**ファイル**: `src/commands/spec/delete.ts`

**実装内容**:
1. 139-146 行目のコメントアウトされたコードを有効化
2. `getGitHubService()` を呼び出して GitHub API クライアントを取得
3. `githubService.issues.close(owner, repo, githubSync.github_number)` を実行
4. 成功メッセージを表示

**受け入れ基準**:
- `--close-github-issue` フラグ指定時、GitHub Issue が正常にクローズされる
- クローズ後、成功メッセージが表示される

**依存関係**: なし
---

### タスク 2: GitHub Issue クローズ失敗時のロールバック処理

**ファイル**: `src/commands/spec/delete.ts`

**実装内容**:
1. GitHub Issue クローズ処理を try-catch で囲む
2. エラーキャッチ時、エラーメッセージを表示
3. 削除処理をスキップ（データベース削除・ファイル削除を実行しない）
4. `spec.deleted` イベントを発火しない

**受け入れ基準**:
- GitHub Issue クローズ失敗時、適切なエラーメッセージが表示される
- 削除処理がロールバックされる（仕様書が削除されない）

**依存関係**: タスク 1 完了後
---

### タスク 3: GitHub API エラーハンドリング

**ファイル**: `src/commands/spec/delete.ts`

**実装内容**:
1. 404 エラー（Issue が存在しない）の場合、警告表示後に削除続行
2. 401 エラー（認証失敗）の場合、エラーメッセージ表示して削除スキップ
3. 403 エラー（レート制限超過）の場合、エラーメッセージ表示して削除スキップ
4. 500 エラー（GitHub API 障害）の場合、エラーメッセージ表示して削除スキップ

**受け入れ基準**:
- 各エラーコードに応じた適切な処理が実行される
- エラーメッセージが明確で、ユーザーが対処方法を理解できる

**依存関係**: タスク 2 完了後
---

### タスク 4: GITHUB_TOKEN 未設定時のエラーメッセージ表示

**ファイル**: `src/commands/spec/delete.ts`

**実装内容**:
1. `process.env.GITHUB_TOKEN` が未設定の場合、エラーメッセージを表示
2. 削除処理をスキップ（GitHub Issue クローズを試みない）

**受け入れ基準**:
- `GITHUB_TOKEN` が未設定の場合、適切なエラーメッセージが表示される
- 削除処理がスキップされる
...
**実装内容**:
1. `--close-github-issue` 指定時、GitHub Issue が正常にクローズされるテスト
2. GitHub Issue クローズ失敗時、削除処理がロールバックされるテスト
3. GitHub Issue が存在しない場合（404エラー）、警告表示後に削除続行するテスト
4. `GITHUB_TOKEN` が未設定の場合、エラーメッセージが表示されるテスト
5. `--yes` フラグ指定時も、GitHub Issue クローズが実行されるテスト
...
**ファイル**: `tests/integrations/github/issues.test.ts`

**実装内容**:
1. `GitHubIssues.close()` が Octokit の `update()` を正しく呼び出すテスト
2. GitHub API レート制限エラーに対応するテスト
3. 認証エラー時、適切なエラーメッセージを返すテスト

**受け入れ基準**:
- すべてのテストケースがパスする
- GitHub API のモック化が正しく実装されている

**依存関係**: タスク 5 完了後
---

### タスク 7: TypeScript 型チェック・ESLint チェック

**実装内容**:
1. `npx tsc --noEmit` で型エラーがないことを確認
2. `npm run lint` で ESLint 警告がないことを確認
3. 必要に応じて型定義を修正
...
**実装内容**:
1. テスト用仕様書を作成
2. `/cft:spec-delete <spec-id> --close-github-issue` を実行
3. GitHub Issue が正常にクローズされることを確認
4. エラーケース（トークン未設定、Issue 存在しない）を確認
