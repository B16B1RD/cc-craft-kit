...
- [ ] `src/core/storage/` ディレクトリとエントリポイント作成 (#849)
- [ ] `src/core/storage/json-storage.ts` 共通 JSON I/O ユーティリティ実装 (#850)
- [ ] `src/core/storage/schemas.ts` Zod スキーマ定義（specs, github-sync, workflow-state, tasks, logs） (#851)
- [ ] `src/core/storage/specs-storage.ts` 仕様書メタデータ管理実装 (#852)
- [ ] `src/core/storage/github-sync-storage.ts` GitHub 同期状態管理実装 (#853)
- [ ] `src/core/storage/workflow-state-storage.ts` ワークフロー状態管理実装 (#854)
- [ ] `src/core/storage/tasks-storage.ts` タスク情報管理実装 (#855)
- [ ] `src/core/storage/logs-storage.ts` ログ管理実装（JSONL 形式） (#856)
- [ ] JSON ストレージモジュールの単体テスト作成 (#857)

### Phase 2: コマンド層の移行

- [ ] `src/commands/spec/list.ts` を JSON ストレージに移行 (#858)
- [ ] `src/commands/spec/get.ts` を JSON ストレージに移行 (#859)
- [ ] `src/commands/spec/register.ts` を JSON ストレージに移行 (#860)
- [ ] `src/commands/spec/delete-query.ts` を JSON ストレージに移行 (#861)
- [ ] `src/commands/spec/delete-execute.ts` を JSON ストレージに移行 (#862)
- [ ] `src/commands/spec/update.ts` を JSON ストレージに移行 (#863)
- [ ] `src/commands/spec/update-phase.ts` を JSON ストレージに移行 (#864)
- [ ] `src/commands/spec/update-pr.ts` を JSON ストレージに移行 (#865)
- [ ] `src/commands/spec/resolve-id.ts` を JSON ストレージに移行 (#866)
- [ ] `src/commands/task/start.ts` を JSON ストレージに移行 (#867)
- [ ] `src/commands/task/done.ts` を JSON ストレージに移行 (#868)
- [ ] `src/commands/workflow/save-state.ts` を JSON ストレージに移行 (#869)
- [ ] `src/commands/workflow/restore-state.ts` を JSON ストレージに移行 (#870)

### Phase 3: インテグレーション層

- [ ] `src/commands/github/create-sub-issues.ts` を github-sync.json に移行 (#871)
- [ ] `src/commands/github/issue-create.ts` を github-sync.json に移行 (#872)
- [ ] `src/commands/github/pr-cleanup.ts` を github-sync.json に移行 (#873)
- [ ] `src/commands/github/project-add.ts` を github-sync.json に移行 (#874)
- [ ] `src/commands/github/sync.ts` を github-sync.json に移行 (#875)
- [ ] `src/commands/status/*.ts` を JSON ストレージに移行 (#876)
- [ ] `src/commands/sync/check.ts` を JSON ベースに移行 (#877)
- [ ] `src/commands/sync/repair.ts` を JSON ベースに移行 (#878)

### Phase 4: クリーンアップ

- [ ] `src/core/database/` ディレクトリ全削除 (#879)
- [ ] `src/core/validators/database-integrity-checker.ts` 削除 (#880)
- [ ] `src/commands/db/info.ts` 削除または JSON 情報表示に変更 (#881)
- [ ] `package.json` から kysely, better-sqlite3, @types/better-sqlite3 を削除 (#882)
- [ ] マイグレーションスクリプト作成（npm run migrate:db-to-json） (#883)
- [ ] `tests/helpers/db-lifecycle.ts` を JSON ライフサイクルに置き換え (#884)
- [ ] 全テスト実行・パス確認 (#885)
- [ ] ドキュメント更新（README, ARCHITECTURE.md） (#886)
