...
- [ ] YAML フロントマター形式の仕様書パーサーを設計 (#906)
- [ ] `/cft:spec` 統合コマンドを新規実装（create/list/get/phase/delete） (#907)
- [ ] `/cft:github` 統合コマンドを `gh` CLI ベースで新規実装（init/issue/sync/cleanup） (#908)
- [ ] `/cft:review` 統合コマンドを新規実装（code-review/lint-check/refactor） (#909)
- [ ] `/cft:test` 統合コマンドを新規実装（test-generate/schema-validate） (#910)
- [ ] `/cft:session` 統合コマンドを新規実装（start/end） (#911)
- [ ] `/cft:status` を TypeScript 依存なしで書き換え (#912)
- [ ] `/cft:task` を TypeScript 依存なしで書き換え (#913)
- [ ] `/cft:sync` を TypeScript 依存なしで書き換え (#914)
- [ ] `/cft:knowledge` を TypeScript 依存なしで書き換え (#915)
