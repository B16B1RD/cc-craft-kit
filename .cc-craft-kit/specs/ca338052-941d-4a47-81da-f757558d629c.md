...
- [ ] `validation.ts` に `isGitHubIssueNumber()` 関数を追加
- [ ] `validation.ts` に `parseGitHubIssueNumber()` 関数を追加
- [ ] `validation.ts` に `isSpecId()` 関数を追加
- [ ] 検証関数の単体テストを追加

### Phase 2: 解決ロジックの拡張

- [ ] `resolve-id.ts` に入力形式の自動判別ロジックを追加
- [ ] `resolve-id.ts` に `resolveSpecByIssueNumber()` 関数を追加
- [ ] GitHub Issue 番号による解決の統合テストを追加

### Phase 3: 動作確認

- [ ] `npm run sync:dogfood` で同期
- [ ] 型チェック・リント実行
- [ ] 手動での動作確認（`/cft:spec-phase #XX design`）
