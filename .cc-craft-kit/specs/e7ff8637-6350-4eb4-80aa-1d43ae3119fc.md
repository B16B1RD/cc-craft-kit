...
- [ ] github-integration.ts: task.completed ハンドラーの開始・完了ログ追加
- [ ] sub-issues.ts: handleTaskCompletion() のデバッグログ追加
- [ ] sub-issues.ts: Sub Issue 未登録時の詳細ログ（登録済み一覧表示）追加
- [ ] sub-issues.ts: parent_issue_number null 時の警告ログ追加
