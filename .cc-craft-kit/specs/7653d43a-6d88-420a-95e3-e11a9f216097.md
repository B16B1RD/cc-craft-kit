---
id: "7653d43a-6d88-420a-95e3-e11a9f216097"
name: "コマンドの棚卸"
phase: "completed"
branch_name: "main"
github_issue_number: null
pr_url: null
created_at: "2025-11-28T01:53:48.132Z"
updated_at: "2025-11-28T12:10:51Z"
---
...
- [x] `src/slash-commands/task.md` を新規作成（サブコマンドルーター実装）
- [x] task.md にサブコマンド分岐ロジックを実装（list/start/update/done/split/report）
- [x] 動作確認: `/cft:task list`, `/cft:task start`, `/cft:task done` をテスト
- [x] 旧コマンドファイル削除（task-list.md, task-start.md, task-update.md, task-done.md, task-split.md, test-report.md）

### Phase 2: 知識記録コマンド統合

- [x] `src/slash-commands/knowledge.md` を新規作成（サブコマンドルーター実装）
- [x] knowledge.md にサブコマンド分岐ロジックを実装（progress/error/tip）
- [x] 動作確認: `/cft:knowledge progress`, `/cft:knowledge error`, `/cft:knowledge tip` をテスト
- [x] 旧コマンドファイル削除（knowledge-progress.md, knowledge-error.md, knowledge-tip.md）

### Phase 3: 品質管理コマンド統合

- [x] `src/slash-commands/quality.md` を新規作成（サブコマンドルーター実装）
- [x] quality.md にサブコマンド分岐ロジックを実装（init/check/generate）
- [x] 動作確認: `/cft:quality init`, `/cft:quality check`, `/cft:quality generate` をテスト
- [x] 旧コマンドファイル削除（quality-init.md, quality-check.md, quality-generate.md）

### Phase 4: 同期コマンド統合

- [x] `src/slash-commands/sync.md` を新規作成（サブコマンドルーター実装）
- [x] sync.md にサブコマンド分岐ロジックを実装（check/repair）
- [x] 動作確認: `/cft:sync check`, `/cft:sync repair` をテスト
- [x] 旧コマンドファイル削除（sync-check.md, sync-repair.md）

### Phase 5: スキル・エージェントコマンド統合

- [x] `src/slash-commands/custom-tools.md` を新規作成（サブコマンドルーター実装）
- [x] custom-tools.md にサブコマンド分岐ロジックを実装（skill list/create, agent list/create）
- [x] 動作確認: `/cft:custom-tools skill list`, `/cft:custom-tools agent create` をテスト
- [x] 旧コマンドファイル削除（skill-list.md, skill-create.md, agent-list.md, agent-create.md）

### Phase 6: 不要コマンド削除・status 拡張

- [x] watch.md を削除
- [x] db-info.md を削除
- [x] migrate-tasks-to-design.md を削除
- [x] github-project-add.md を削除
- [x] status.md に `--verbose` オプションを追加（DB 情報表示機能）
- [x] `npm run sync:dogfood` で同期
- [x] 最終動作確認（仕様書作成→タスク管理→完了の E2E フロー）
- [x] コマンド数が 30 個以下であることを確認（結果: 24 個）
