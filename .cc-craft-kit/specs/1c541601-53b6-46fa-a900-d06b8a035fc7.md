...
- [ ] Query 層スクリプト (`delete-query.ts`) を作成
  - ID 解決（部分一致、8文字以上）
  - 削除前情報取得（JSON 出力）
- [ ] Execute 層スクリプト (`delete-execute.ts`) を作成
...
- [ ] Prompt 層 (`spec-delete.md`) をリファクタリング
  - 制御フロー管理
  - ユーザー確認（AskUserQuestion）
  - 結果表示・エラー解析
  - ガイダンスメッセージ
...
- [ ] Query 層の単体テスト (`delete-query.test.ts`) を作成
- [ ] Execute 層の単体テスト (`delete-execute.test.ts`) を作成
