...
async function main() {
  ...
  for (const record of invalidRecords) {
    ...
    if (isNaN(githubNumber)) {
      console.log(`⚠️  Skipping record ${record.id}: github_id "${record.github_id}" is not a number`);
      continue;
    }
    ...
    console.log(`✓ Fixed ${record.entity_type} ${record.entity_id.substring(0, 8)}... (Issue #${githubNumber})`);
    fixedCount++;
  }
  ...
  if (duplicates.length > 0) {
    ...
    for (const dup of duplicates) {
      console.log(`  - ${dup.entity_type} ${dup.entity_id.substring(0, 8)}... (${dup.count} records)`);
