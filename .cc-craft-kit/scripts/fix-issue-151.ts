...
async function main() {
  ...
  console.log(`✅ ${deleteResult.length > 0 ? deleteResult[0].numDeletedRows : 0} 件のレコードを削除しました`);
  ...
  updatedSyncRecords.forEach((record) => {
    console.log(`  - GitHub ID: ${record.github_id}, GitHub Number: ${record.github_number}, Sync Status: ${record.sync_status}`);
  });
