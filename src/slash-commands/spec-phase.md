...
**重要**: フェーズ移行コマンドが完了した後、以下の処理を**自動的に実行**してください。

### 1. Git 状態確認

Bash ツールで `git status --porcelain` を実行し、未コミットファイルの有無を確認します。

### 2. 未コミットファイルの検出と警告

出力が空でない場合、未コミットファイルが存在します。以下の警告メッセージを表示してください:
...
未コミットファイルがない場合、以下のメッセージを表示してください:
